...
RUN apt-get update \
    && apt-get install -y slapd wget tar unzip dos2unix
...
RUN set -x; \
    ...
    && touch start.ini \
    && /opt/jdk1.7.0_75/bin/java -jar ../jetty/start.jar --add-to-startd=http,https,deploy,ext,annotations,jstl,logging,jaas
 
RUN unzip apache-ant-1.9.4-bin.zip -d /opt \
...
RUN echo "JETTY_HOME=/opt/jetty/" >> /etc/default/jetty \
    && echo "JETTY_BASE=/opt/iam-jetty-base/" >> /etc/default/jetty \
    && echo "JAVA=/opt/jdk1.7.0_75/bin/java" >> /etc/default/jetty
